...
import {
  ...
  meetingCreateSchema,
  meetingIntegrationCreateSchema,
...
describe("taskUpdateSchema", () => {
  it("does not add create defaults to a partial update", () => {
    expect(taskUpdateSchema.parse({ status: "review", version: 3 })).toEqual({
      status: "review",
      version: 3,
    });
  });
  ...
  it("validates milestones, dependency cycles at the boundary and versioned views", () => {
    ...
    expect(
      dependencyCreateSchema.safeParse({
  ...
  it("keeps automation actions bounded and meeting consent explicit", () => {
    ...
    expect(
      automationCreateSchema.safeParse({
        ...
        actions: [{ type: "notification.send", config: { title: "Neu" } }],
      }).success,
    ).toBe(true);
    expect(
  ...
  it("validates meeting bot credentials without accepting partial setups", () => {
    const workspaceId = crypto.randomUUID();
    expect(
      meetingIntegrationCreateSchema.safeParse({
        workspaceId,
        provider: "discord",
        name: "Scope Bot",
        applicationId: "123456789012345678",
        botToken: "x".repeat(40),
      }).success,
    ).toBe(true);
    expect(
      meetingIntegrationCreateSchema.safeParse({
